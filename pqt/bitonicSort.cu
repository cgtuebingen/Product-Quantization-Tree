...
////////////////////////////////////////////////////////////////////////////////
__global__ void bitonicSortShared(float *d_DstKey, uint *d_DstVal,
		float *d_SrcKey, uint *d_SrcVal, uint arrayLength, uint dir) {
	...
	d_DstKey[(SHARED_SIZE_LIMIT / 2)] = s_key[threadIdx.x
			+ (SHARED_SIZE_LIMIT / 2)];
	d_DstVal[(SHARED_SIZE_LIMIT / 2)] = s_val[threadIdx.x
			+ (SHARED_SIZE_LIMIT / 2)];
}
...
__global__ void bitonicSortShared1(float *d_DstKey, uint *d_DstVal,
		float *d_SrcKey, uint *d_SrcVal) {
...
__global__ void bitonicMergeGlobal(float *d_DstKey, uint *d_DstVal,
		float *d_SrcKey, uint *d_SrcVal, uint arrayLength, uint size,
...
__global__ void bitonicMergeShared(float *d_DstKey, uint *d_DstVal,
		float *d_SrcKey, uint *d_SrcVal, uint arrayLength, uint size,
		uint dir) {
	//Shared memory storage for current subarray
	__shared__ float s_key[SHARED_SIZE_LIMIT];
	...
	//Bitonic merge
	uint comparatorI = UMAD(blockIdx.x, blockDim.x, threadIdx.x)
			& ((arrayLength / 2) - 1);
	uint ddd = dir ^ ((comparatorI & (size / 2)) != 0);
...
//extern "C"
uint bitonicSort(float *d_DstKey, uint *d_DstVal, float *d_SrcKey,
		uint *d_SrcVal, uint batchSize, uint arrayLength, uint dir) {
	//Nothing to sort
	if (arrayLength < 2)
		return 0;
	...
	if (arrayLength <= SHARED_SIZE_LIMIT) {
		...
		bitonicSortShared<<<blockCount, threadCount>>>(d_DstKey, d_DstVal,
				d_SrcKey, d_SrcVal, arrayLength, dir);
		...
		for (uint size = 2 * SHARED_SIZE_LIMIT; size <= arrayLength; size <<= 1)
			for (unsigned stride = size / 2; stride > 0; stride >>= 1)
				if (stride >= SHARED_SIZE_LIMIT) {
					...
					bitonicMergeShared<<<blockCount, threadCount>>>(d_DstKey,
							d_DstVal, d_DstKey, d_DstVal, arrayLength, size,
							dir);
					break;
				}
	}
